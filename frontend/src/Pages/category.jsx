    ...
    return (
      <div className="grid w-11/12 mx-auto grid-cols-[50px,1fr,50px] my-10">
        ...
        <div className="overflow-auto relative">
          ...
          <div
            className="relative w-full flex gap-6 snap-x snap-mandatory scroll-smooth scrollbar-hide"
          ...
          >
            {x.map((dt, id) => {
              return (
                <Link
                  to={`/article/${dt.id}`}
                  className="w-[500px] h-[250px] relative overflow-hidden rounded-xl cursor-pointer snap-start shrink-0"
                  key={`article_prority${id}`}
                >
                  <img
                    src={dt.image}
        ...
        <div className="flex items-center justify-center">
          <Button
            shape="circle"
            icon={
              <Icon icon={"solar:alt-arrow-right-broken"} className="text-xl" />
            }
            onClick={() => {
              checkScroll(500);
            }}
          ...
          {Artikel.filter((dt) => dt.priority == false).map((dt, id) => {
            return (
              <Link to={`/article/${dt.id}`} key={`article_${id}`}>
                <div
                  className="grid grid-cols-[300px,1fr] gap-10"
                  key={`article_${id}`}
                >
                  <img
                    src={dt.image}
                ...
                {id !=
                  Artikel.filter((dt) => dt.priority == false).length - 1 && (
